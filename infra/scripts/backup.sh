...
source "$APP_DIR/stack.env"   # ENVIRONMENT, REGION, BACKUP_BUCKET, SITE_NAME, COMPOSE_PROJECT

STAMP="$(date -u +%Y-%m-%dT%H-%M-%SZ)"
PREFIX="s3://$BACKUP_BUCKET/$ENVIRONMENT/$STAMP"

echo "[backup] creating dump for $SITE_NAME"
docker compose -p "$COMPOSE_PROJECT" -f compose.yaml exec -T backend \
  bench --site "$SITE_NAME" backup --with-files

# bench writes into sites/<site>/private/backups inside the sites volume.
BACKUP_SRC="/var/lib/docker/volumes/${COMPOSE_PROJECT}_sites/_data/$SITE_NAME/private/backups"
